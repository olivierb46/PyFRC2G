"""
XML backup parser for pfSense and OPNSense firewall configurations.
Reads config.xml backup files and extracts rules + aliases in API-compatible format.
"""
...
def _get_text(element, default=""):
    """Get text from XML element, handling None."""
    if element is None:
        return default
...
def get_gateway_name_from_xml(xml_path):
    """
    Extract gateway name from backup XML as the system hostname.
...
def _extract_address(source_or_dest):
    """
    Extract address from source/destination XML element.
    Handles: <any/>, <any></any>, <any>1</any>, <network>X</network>, <address>X</address>.
    """
    if source_or_dest is None:
        return None
    # Any: self-closing, empty, or <any>1</any> (OPNSense) -> always "any"
...
def _extract_port(dest_elem):
    """Extract port from destination element (e.g. <destination><port>53</port></destination>)."""
...
def parse_pfsense_backup(xml_path):
    ...
    Returns (aliases_dict, rules_list) compatible with API format.
    """
    tree = ET.parse(xml_path)
    root = tree.getroot()
    ...
    if aliases is not None:
        for alias in aliases.findall("alias"):
            ...
            # Address can be single or multiple (pf uses 'address' or 'address' with newlines)
    ...
    # Parse rules: /pfsense/filter/rule or /pfsense/filter/rule (floating in separate section)
    rules = []
    ...
    if floating is not None:
        for rule in floating.findall("rule"):
            ...
            if entry:
                entry["floating"] = True
                entry["interface"] = None  # Floating = no interface
                rules.append(entry)
...
def _parse_pfsense_rule(rule_elem):
    """Parse a single pfSense rule element into API-compatible dict."""
    ...
    # Port can be in <destination><port>53</port></destination>
...
def parse_opnsense_backup(xml_path):
    ...
    In backup XML, filter is at root: <opnsense><filter><rule>...</rule></filter>.
    Returns (aliases_dict, rules_list) compatible with API format.
    """
    tree = ET.parse(xml_path)
    root = tree.getroot()
    ...
    # Parse aliases: try /opnsense/firewall/aliases then nested OPNsense/firewall/aliases
    ...
    # Parse rules: in backup config.xml filter is directly under root <opnsense><filter>
    rules = []
    filter_elem = root.find("filter")
    if filter_elem is not None:
        for rule in filter_elem.findall("rule"):
...
def parse_xml_backup(xml_path):
    ...
    Returns:
        ...
        aliases_dict has keys: interface_map, net_map, address_map, port_map, alias_details
        rules_list is a list of rule dicts in API-compatible format
        gateway_name is hostname from XML, or None if not found
    """
    path = Path(xml_path)
    if not path.exists():
